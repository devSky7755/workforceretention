...
export class BlogComponent implements OnInit {
  ...
  page(offset, limit) {
    this.articleService.getArticles(offset, limit, true).subscribe(
      (results) => {
        ...
        this.articles = this.articles.map((article) => {
          ...
          article.imageSrc =
            this.urlService.baseUrl + "/images/article/" + article.image;
  ...
  getExcerpt(html) {
    const length = 200;
    const excerpt = this.htmlToPlaintext(html);
