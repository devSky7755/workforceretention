...
export class QuestionsComponent implements OnInit, AfterViewInit {
  ...
  pageName;
  employee_details;
  survey_types = [{ id: 1, value: "Exit Interview" }];
  ratings = [
    { id: 2, value: "1-2" },
    { id: 3, value: "1-3" },
    { id: 4, value: "1-4" },
  ...
  ngOnInit() {
    ...
    this.pageName =
      "Your Exit Interview " +
      this.employee_details.first_name +
      " " +
      this.employee_details.last_name;
    this.getSurvey();
  }
  ...
  getSurvey() {
    ...
    this.employeeService
      .getEmployeeSurveys(this.employee.employee_id)
      .subscribe((data) => {
        ...
        if (this.employeeSurvey) {
          ...
          this.surveyStatus = this.employeeSurvey.completed
            ? "Completed"
            : this.employeeSurvey.start_date
            ? "In Progress"
            : "Not Started";
          this.cur_step = this.employeeSurvey.exit_reason_cur_step
            ? parseInt(this.employeeSurvey.exit_reason_cur_step)
            : 11;
  ...
  setPage(data) {
    ...
    this.survey.type_label = this.survey_types.find(
      (s) => s.id == this.survey.survey_type
    ).value;
    this.survey.rating_label = this.ratings.find(
      (r) => r.id == this.survey.rating_scale
    ).value;
  ...
  questionArrange() {
    ...
    this.exit_reason.map((reason, index) => {
      ...
      const val = {
        ...
        exit_reason_id: reason.id,
        questions: categorical_questions,
        step_no: step_no,
      };
      this.categorical_questions.push(val);
      const cur_step = this.cur_section_step || this.cur_step;
      if (
        this.exit_reason_ids.indexOf(cur_step) <= index &&
        !this.cur_categorical_questions
      ) {
  ...
  setQuestionAnswer() {
    this.answerService
      .getEmployeeSurveyAnswer(this.employee.employee_id, this.surveyId)
      .subscribe((data) => {
  ...
  setAnswer() {
    this.cur_categorical_questions.questions.map((question) => {
      ...
      if (!this.isNullOrEmpty(answer)) {
        ...
        if (
          answer.question_type == this.question_types[0].id &&
          answer.question_type != "7"
        ) {
          // Rating Radio Buttons
          this.survey.rating_labels.map((label, index) => {
            const rating_radio_input = <HTMLInputElement>(
              document.getElementById(
                `rating-radio-label-${question.question_no}-${index}`
              )
            );
          ...
          answer.question_type == this.question_types[1].id &&
          answer.question_type != "7"
        ) {
          // Free Text
          const free_text_input = <HTMLInputElement>(
            document.getElementById(`question-comment-${question.question_no}`)
          );
          free_text_input.value = answer.options[0];
        } else if (
          answer.question_type == this.question_types[2].id &&
          answer.question_type != "7"
        ) {
          // Exit Interview - Exit Reasons
          question.options.map((option, index) => {
            if (option == "true") {
              const simple_radio_input = <HTMLInputElement>(
                document.getElementById(
                  `exit-reason-choice-${question.question_no}-${index}`
                )
              );
          ...
          answer.question_type == this.question_types[3].id &&
          answer.question_type != "7"
        ) {
          // Yes / No Radio
          const yes_radio_label = <HTMLInputElement>(
            document.getElementById(`radio-${question.question_no}-yes`)
          );
          const no_radio_label = <HTMLInputElement>(
            document.getElementById(`radio-${question.question_no}-no`)
          );
          ...
          answer.question_type == this.question_types[4].id &&
          answer.question_type != "7"
        ) {
          // Radio Labels
          question.options.map((option, index) => {
            const radio_label_input = <HTMLInputElement>(
              document.getElementById(
                `radio-label-${question.question_no}-${index}`
              )
            );
            if (answer.options[0] == index) {
          ...
          this.exit_reason_checkbox.map((exit_reason, index) => {
            ...
            if (question.options[index] == "true") {
              const first_choice_radio_input = <HTMLInputElement>(
                document.getElementById(
                  `final-1st-choice-${question.question_no}-${exit_reason.id}`
                )
              );
              const second_choice_radio_input = <HTMLInputElement>(
                document.getElementById(
                  `final-2nd-choice-${question.question_no}-${exit_reason.id}`
                )
              );
          ...
          // Multiple Choice
          question.options.map((option, index) => {
            const multiple_choice_input = <HTMLInputElement>(
              document.getElementById(
                `multiple-choice-${question.question_no}-${index}`
              )
            );
            const option_index = "" + index;
            if (answer.options.includes(option_index)) {
  ...
  onSubmitAnswer() {
    ...
    this.cur_categorical_questions.questions.map((question) => {
      ...
      const options = [];
      if (
        question.type == this.question_types[0].id &&
        this.cur_categorical_questions.exit_reason !== "Final Question"
      ) {
        // Rating Radio Buttons
        let valid = false;
        this.survey.rating_labels.map((label, index) => {
        ...
        question.type == this.question_types[1].id &&
        this.cur_categorical_questions.exit_reason !== "Final Question"
      ) {
        // Free Text
        const free_text_input = <HTMLInputElement>(
        ...
        const comment =
          free_text_input.value === "" ? " " : free_text_input.value;
        options.push(comment);
      } else if (
        question.type == this.question_types[2].id &&
        this.cur_categorical_questions.exit_reason !== "Final Question"
      ) {
        let valid = false;
        // Exit Interview - Exit Reasons
        ...
        question.type == this.question_types[3].id &&
        this.cur_categorical_questions.exit_reason !== "Final Question"
      ) {
        let valid = false;
        // Yes / No Radio
        ...
        question.type == this.question_types[4].id &&
        this.cur_categorical_questions.exit_reason !== "Final Question"
      ) {
        // Radio Labels
        let valid = false;
    ...
    if (errors.length == 0 && cur_step == this.last_step) {
      // alert('Please select an option for question -> ' + errors[0]);
      this.confirmRef = this.dialogService.open(this.confirm, {
        hasBackdrop: false,
      });
      return;
    } else {
  ...
  confirmComplete(val) {
    ...
    if (this.confirmRef) this.confirmRef.close();
  }

  prevCategoricalQuestions() {
    const index = this.categorical_questions.findIndex((cat_questions) => {
      return (
        cat_questions.exit_reason_id ==
        this.cur_categorical_questions.exit_reason_id
      );
    });
  ...
  saveQuestionAnswer() {
    const next_exit_reason_id =
      this.nextStopCategoricalQuestions().exit_reason_id;
    this.answerService
      .createManyAnswer(
        this.question_answers,
        this.surveyId,
        ...
        "Yes",
        "No",
        this.is_complete_submit,
        next_exit_reason_id
      )
      .subscribe(() => {
        if (this.is_complete_submit) {
          alert(
            "Thank you for taking the time to complete your online exit interview"
          );
          this.router.navigateByUrl("/client/dashboard");
          return;
        } else {
          this.cur_section_step =
            this.nextCategoricalQuestions().exit_reason_id;
          this.getSurvey();
        }
      });
  }
  ...
  updateQuestionAnswer() {
    ...
    this.answers.map((answer) => {
      const selected_question = this.question_answers.find(
