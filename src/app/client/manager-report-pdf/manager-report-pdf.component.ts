...
export class ManagerReportPdfComponent implements OnInit {
  ...
  constructor(private employeeService: EmployeeService, private reportService: ReportService, private route: ActivatedRoute,
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      this.filterData = {
        start_date: params['start_date'] == 'null' || !params['start_date'] ? null : params['start_date'],
        end_date: params['end_date'] == 'null' || !params['end_date'] ? null : params['end_date'],
  ...
  getManagerReport() {
    this.reportService.getReport(this.employeeId, this.filterData).subscribe(
      res => {
  ...
  showChartReport(data) {
    ...
    if (show_data) {
      ...
      if (total !== 0) {
        this.gender_split_chart_data.filter((g) => {
          return g.value != 0
        }).map((g) => {
