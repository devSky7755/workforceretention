...
export class EmployeeSurveyDownloadComponent implements OnInit {
    ...
    setPage(data) {
        ...
        this.clientImage = this.employee.client.image
            ? this.baseUrl + "/images/client/" + this.employee.client.image
    ...
    getStateLabel(id) {
        const matchedState = this.states.find((state) => state.id == id);
        return (matchedState && matchedState.value) || "";
    }
}
