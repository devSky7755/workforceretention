...
export class ManagerReportComponent implements OnInit {
    ...
    exit_reason_data_mapper = [
        { exit_reason: "1", value: this.career_opportunities_chart_data, canvasData: [] },
        { exit_reason: "2", value: this.meaningful_work_chart_data, canvasData: [] },
        { exit_reason: "3", value: this.communication_chart_data, canvasData: [] },
        { exit_reason: "4", value: this.effective_leadership_chart_data, canvasData: [] },
        { exit_reason: "5", value: this.induction_chart_data, canvasData: [] },
        { exit_reason: "6", value: this.learning_development_chart_data, canvasData: [] },
        { exit_reason: "7", value: this.manager_chart_data, canvasData: [] },
        { exit_reason: "8", value: this.pay_benefits_chart_data, canvasData: [] },
        { exit_reason: "9", value: this.work_conditions_chart_data, canvasData: [] },
    ...
    constructor(private reportService: ReportService, private urlService: URLService) {
        this.manager = {};
        this.client = {};
    }
    ...
    getManagerReport() {
        this.reportService.getReport(this.employee.employee_id, this.filterData).subscribe((res) => {
    ...
    getManagerDetails() {
        this.reportService.getReportDetails(this.employee.employee_id).subscribe((res) => {
    ...
    showChartReport(data) {
        ...
        if (show_data) {
            ...
            if (total !== 0) {
                ...
                this.gender_split_chart_data = this.gender_split_chart_data.filter((g) => {
                    return g.value != 0;
                });
            } else {
                this.gender_split_chart_data = [];
            ...
            if (total !== 0) {
                ...
                this.tenure_split_chart_data = this.tenure_split_chart_data.filter((t) => {
                    return t.value != 0;
                });
            } else {
            ...
            final_question.options.sort((a, b) => (a.label == "true" && b.label == "false" ? 1 : -1));
            final_question.options.sort((a, b) => (parseFloat(a.percentage) < parseFloat(b.percentage) ? 1 : -1));
                ...
                filtered_reason.map((reason) => {
                    ...
                    reason.options.map((r, index) => {
                        if (reason.question_type === "3") {
                            ...
                            if (
                                positive_values.findIndex((item) => r.label.toLowerCase() === item.toLowerCase()) > -1
                            ) {
                                positive += r.answered;
                            } else if (
                                negative_values.findIndex((item) => r.label.toLowerCase() === item.toLowerCase()) > -1
                            ) {
                                negative += r.answered;
                            } else {
                                neutral += r.answered;
            ...
            rating_radio_button_questions.map((question) => {
                ...
                this.employee_sentiment.push({
                    name:
                        this.exit_reasons.find((ex) => ex.id == question.exit_reason).value +
                        " - " +
            ...
            if (this.employee_sentiment_working_chart_data.length > 0) {
                ...
                for (let i = 0; i < this.employee_sentiment_working_chart_data[0].series.length; i++) {
                    ...
                    for (let j = 0; j < this.employee_sentiment_working_chart_data.length; j++) {
                        if (this.employee_sentiment_working_chart_data[j].series[i].value == 0) continue;
                        canvasData.dataPoints.push({
            ...
            if (this.employee_sentiment_not_working_chart_data.length > 0) {
                for (let i = 0; i < this.employee_sentiment_not_working_chart_data[0].series.length; i++) {
                    ...
                    for (let j = 0; j < this.employee_sentiment_not_working_chart_data.length; j++) {
                        if (this.employee_sentiment_not_working_chart_data[j].series[i].value == 0) continue;
                        canvasData.dataPoints.push({
                            label: this.employee_sentiment_not_working_chart_data[j].name,
                            y: this.employee_sentiment_not_working_chart_data[j].series[i].value,
                        });
                    }
    ...
    onDownloadPdf() {
        ...
        const url = `/client/manager-report-pdf?employeeId=${this.employee.employee_id}&start_date=${this.filterData.start_date}&end_date=${this.filterData.end_date}&level=${this.filterData.level}&occupational_group=${this.filterData.occupational_group}&gender=${this.filterData.gender}&tenure=${this.filterData.tenure}&state=${this.filterData.state}`;
        // window.open(url, '_blank');
        this.reportService.downloadManagerReport({ url }).subscribe((res) => {
