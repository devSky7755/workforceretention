...
export class ClientSelectionComponent implements OnInit {
  ...
  limit = 10;
  clients;
  products = [{ id: 1, name: "Exit Interview" }];
  workforces = [
  ...
  onClickDelete(id) {
    ...
    if (client.employees.length > 0) {
      if (
        confirm(
          "There are employees assigned to this Client, are you sure you want to delete ? " +
            name,
        )
      ) {
  ...
  page(offset, limit) {
    this.tokenService.get().subscribe((token) => {
      ...
      this.clientService.getClients(offset, limit, payload._id).subscribe(
        (data) => {
          ...
          this.clients.map((client) => {
            ...
            client.industryName =
              typeof client.industry === "undefined" || client.industry == null
                ? ""
                : client.industry.name;
            client.product = this.products.find(
              (p) => p.id === client.product,
            ).name;
            client.workforce = this.workforces.find(
              (w) => w.id === client.workforce,
            ).value;
            rows.push(client);
          });
          this.rows = rows;
