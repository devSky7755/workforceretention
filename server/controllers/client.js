...
exports.Create = function (req, res, next) {
  ...
  const { error, value } = clientSchema.validate(data);
      ...
      .then((user) => {
        ...
        User.findById(userId, "-password -surveys -links")
          ...
          .then((user) => {
            return res
              .status(200)
              .json({ success: true, clients: user.clients, totalItems });
          })
          .catch((err) => {
            if (!err.statusCode) {
              err.statusCode = 500;
...
exports.FindById = (req, res, next) => {
  ...
  Client.findById(id)
    ...
    .exec(function (err, client) {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Data successfully retrieve", client });
    });
};
...
exports.FindEmployees = (req, res, next) => {
  ...
  Client.findById(clientId)
    ...
    .exec(function (err, client) {
      ...
      return res
        .status(200)
        .json({ success: true, client, employees, totalItems });
    });
};
...
exports.AssignSurvey = (req, res, next) => {
  ...
  Client.findById(clientId).then((client) => {
    ...
    employeeAssignSurvey(client.employees, surveyId)
      .then((employees) => {
        client.save();
      })
      .then(() => {
        res.json({
          client,
          success: true,
          message: "Survey successfully assigned",
        });
      })
      .catch((err) => {
        return next(err);
...
exports.UnAssignSurvey = (req, res, next) => {
  ...
  Client.findById(clientId)
    ...
    .exec((err, client) => {
      ...
      if (!completedSurvey) {
        employeeUnAssignSurvey(client.employees, surveyId)
          .then(() => {
            client.save();
          })
          .then(() => {
            return res.json({
              client,
              success: true,
              message: "Survey successfully unAssigned",
            });
          })
          .catch((err) => {
            return next(err);
        ...
        return next(
          new Error(
            "Employee under client " +
              client.name +
              " already completed assign survey. you are not allowed to unAssign survey"
          )
        );
      }
    });
