...
exports.Create = function (req, res, next) {
  ...
  const { error, value } = requestaquoteSchema.validate(data);
  ...
  raq
    ...
    .then((raq) => {
      ...
      body = body.replace(
        "[cur_employee_turnover-val]",
        raq.cur_employee_turnover
      );
      body = body.replace(
        "[confidential]",
        radioTemplate(raq.confidential, "Confidential Program")
      );
      body = body.replace(
        "[license_required_status]",
        radioTemplate(
          raq.license_required_status,
          "License to Workforce Retentions Exit Interview Platform"
        )
      );
      body = body.replace(
        "[phone_interview_with_platform_status]",
        radioTemplate(
          raq.phone_interview_with_platform_status,
      ...
      body = body.replace(
        ...
        radioTemplate(
          raq.phone_interview_without_platform_status,
          "Expert Phone Interviews WITHOUT Platform"
        )
      );
      body = body.replace(
        "[exit_report_status]",
        radioTemplate(raq.exit_report_status, "Comprehensive Exit Reports")
      );
