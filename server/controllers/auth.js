    ...
    jwt.verify(token, config.SECRET, function (err, decoded) {
      if (err) {
        if (err.name === "TokenExpiredError") {
          return res
            .status(401)
            .send({ data: { errors: ["Token is expired."] } });
        } else {
          throw err;
        }
      }
        ...
        User.findOne(id)
          .then(async (user, error) => {
            ...
            if (!user || user.phone !== phone) {
              ...
              return res
                .status(401)
                .send({ data: { errors: ["User not found."] } });
            }
            const accountSid = config.TWILIO_ACCOUNT_SID;
            const authToken = config.TWILIO_AUTH_TOKEN;
            const client = twilio(accountSid, authToken);
              ...
              client.verify
                ...
                .then((verification_check) => {
                  if (verification_check.valid) {
                    ...
                    return res
                      .status(401)
                      .send({ data: { errors: ["Code is incorrect."] } });
                  }
                })
                .catch((err) => {
                  return res
    ...
    .then((user, err) => {
      if (err) {
        return res
          .status(401)
          .send({
            data: { errors: ["Unable to find user with the email" + email] },
          });
      }
      if (!user) {
        return res
          .status(401)
          .send({ data: { errors: ["User not found, please sign up."] } });
      }
...
exports.resetPass = function (req, res, next) {
  ...
  if (password !== confirmPassword) {
    return res
      .status(403)
      .send({
        data: { errors: ["Password does not match the confirm password."] },
      });
  }
