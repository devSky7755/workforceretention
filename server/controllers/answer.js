...
exports.Create = function (req, res, next) {
  ...
  const { error, value } = answerSchema.validate(data);
      ...
      Question.find({ _id: { $in: questions } }, function (err, question_docs) {
        ...
        employee.surveys.forEach((employee_survey) => {
          ...
          if (employee_survey.survey.equals(surveyId)) {
            ...
            employee_survey.completed_online =
              isComplete == 1 ? data.completed_online : "No";
            employee_survey.completed_admin =
              isComplete == 1 ? data.completed_admin : "No";
          }
        });
        employee.save().then(() => {
          res.json({
  ...
  updateNewAnswers(new_answers).then(() => {
    Answer.find({ _id: { $in: answer_ids } }, function (err, answers) {
      ...
      answers.forEach((answer) => {
        answer.options = data.find((a) =>
          mongoose.Types.ObjectId(a.question).equals(answer.question)
        ).options;
        answer.save();
      });
      Employee.findById(employeeId, (err, employee) => {
        ...
        employee.surveys.forEach((employee_survey) => {
          ...
          if (employee_survey.survey.equals(surveyId)) {
            ...
            if (isOnline == "Yes") {
              employee_survey.completed_online =
                isComplete == 1 ? isOnline : "No";
            }
            if (isAdmin == "Yes") {
              employee_survey.completed_admin =
                isComplete == 1 ? isAdmin : "No";
            }
            if (last_edit) {
          ...
          function (err, question_docs) {
            ...
            question_docs.forEach((question) => {
              question.answers.push(
                answers.find((a) =>
                  mongoose.Types.ObjectId(a.question).equals(question._id)
                ).id
              );
              question.save();
            });
