    ...
    .exec(function (err, employee) {
      ...
      Client.findById(employee.client)
        ...
        .exec(function (err, client) {
          ...
          if (err) return next(err);
          if (employee.is_report === "0") {
            message = `${completedSurveys} surveys have been completed by Employees of organization ${
...
exports.ManagerReport = (req, res, next) => {
  ...
  Employee.findById(employeeId, "-password")
    .populate({
      path: "organization",
      model: "Organization",
    })
          ...
          Survey.findById(surveyId)
            ...
            .exec(function (err, survey) {
              ...
              filtered_employees = filtered_employees.filter(
                (e) => e.surveys[0].completed === true
              ); // this will filter all the employees who have completed the survey
              ...
              if (!IsNullOrEmpty(tenure) && tenure !== "") {
                if (tenure === "1") {
                  // < 1 year
                  filtered_employees = filtered_employees.filter(
                    (e) => getAge(e.hire_date) <= 1
                  );
                } else if (tenure === "2") {
                  // 1-2 years
                  filtered_employees = filtered_employees.filter(
                    (e) => getAge(e.hire_date) > 1 && getAge(e.hire_date) <= 2
                  );
                } else if (tenure === "3") {
                  // 3-5 years
                  filtered_employees = filtered_employees.filter(
                    (e) => getAge(e.hire_date) > 2 && getAge(e.hire_date) <= 5
                  );
                } else if (tenure === "4") {
                  // 6-10 years
                  filtered_employees = filtered_employees.filter(
                    (e) => getAge(e.hire_date) > 5 && getAge(e.hire_date) <= 10
                  );
                } else if (tenure === "5") {
                  // greater than 10 > 10 years
                  filtered_employees = filtered_employees.filter(
                    (e) => getAge(e.hire_date) > 10
                  );
                }
              }
              ...
              if (employee.is_report === "0") {
                ...
                filtered_employees = filtered_employees.filter((fe) =>
                  employee.organization._id.equals(fe.organization)
                );
              }
              ...
              employeeQuestionAnswers(filtered_employees).then(
                (employee_answers) => {
                  survey.questions.forEach((question) => {
                    ...
                    const answers = employee_answers.filter((d) =>
                      d.question.equals(question._id)
                    );
                    ...
                    answers.forEach((answer) => {
                      ...
                      if (
                        ...
                        question_object.options.forEach((option) => {
                          if (
                            JSON.parse(answer.options[0]) ===
                            JSON.parse(option.label_index)
                          ) {
                        ...
                        question_object.options.forEach((option) => {
                          ...
                          let first_choice_index =
                            "1st-choice-" + (option.label_index + 1);
                          let second_choice_index =
                            "2nd-choice-" + (option.label_index + 1);
            ...
            if (employee.surveys.length > 0) {
              ...
              if (survey._id.equals(surveyId) && employee.is_manager === "0") {
                let data_object = {
                  ...
                  employee_type:
                    employee.is_manager === "0" ? "Employee" : "Manager",
                  employee_org:
                    employee.organization === null ||
                    typeof employee.organization == "undefined"
                      ? ""
                      : employee.organization.name,
                  employee_div:
                    employee.division === null ||
                    typeof employee.division == "undefined"
                      ? ""
                  ...
                  employee_exitdate:
                    employee.exit_date == null
                      ? ""
                      : format_date(employee.exit_date),
                  employee_gender: employee.gender,
                  employee_note:
                    employee.note == null
                      ? ""
                      : employee.note.replace(/\r/g, "\n"),
                  employee_dob:
                  ...
                  survey_starttime:
                    employee.surveys[0].start_date == null
                      ? ""
                      : format_date(employee.surveys[0].start_date),
                  survey_endtime:
                    employee.surveys[0].end_date == null
                      ? ""
                      : format_date(employee.surveys[0].end_date),
                  completed_online: employee.surveys[0].completed_online,
                  completed_admin: employee.surveys[0].completed_admin,
                  last_completed_at:
                    !employee.surveys[0].completed_online ||
                    !employee.surveys[0].completed_admin ||
                    employee.surveys[0].end_date == null
                      ? ""
                      : format_date_time(employee.surveys[0].end_date),
                  last_edit: employee.surveys[0].last_edit,
                  created_by:
                    employee.created_by === null ||
                    typeof employee.created_by == "undefined"
                      ? ""
                      : `${employee.created_by.first_name} ${employee.created_by.last_name}`,
                };
                let answers = [];
                ...
                survey.questions.forEach((question) => {
                  let answer = answers.find(
                    (a) =>
                      a.employee.equals(employee._id) &&
                      a.question.equals(question._id)
                  );
                  ...
                  if (answer) {
                    ...
                    if (
                      question.type === "1" &&
                      question.exit_reason !== "13"
                    ) {
                      data_object[question_id] = escapeSpecialChars(
                        survey.rating_labels[answer.options[0]]
                      );
                    } else if (
                      question.type === "2" &&
                      question.exit_reason !== "13"
                          ...
                          answer.options.forEach((answer_option) => {
                            if (`${question_index}` === answer_option) {
                              ...
                              final_answer +=
                                i +
                                ". " +
                                exit_reason_checkbox[question_index].value +
                                "\n";
                            }
                          });
                        }
                      });
                      data_object[question_id] =
                        escapeSpecialChars(final_answer);
                    } else if (
                      question.type === "4" &&
                      question.exit_reason !== "13"
                    ) {
                      data_object[question_id] =
                        answer.options[0] === "1" ? "Yes" : "No";
                    } else if (
                      question.type === "5" &&
                      question.exit_reason !== "13"
                    ) {
                      data_object[question_id] = escapeSpecialChars(
                        question.options[answer.options[0]]
                      );
                    } else if (
                      question.type === "6" &&
                      question.exit_reason !== "13"
                      ...
                      if (answer.options.length >= 2) {
                        ...
                        if (answer_one[0] === "1st") {
                          final_answer +=
                            "1st choice " +
                            exit_reason_checkbox.find(
                              (ex) => ex.id == answer_one[2]
                            ).value +
                            "\n";
                        } else {
                          final_answer +=
