...
exports.Create = function (req, res, next) {
  ...
  const { error, value } = questionSchema.validate(data);
  ...
  Survey.findById(surveyId, (err, survey) => {
    ...
    if (!survey) {
      return res
        .status(404)
        .json({ status: false, message: "No survey found!" });
    }
...
exports.UpdateSurveyQuestions = (req, res, next) => {
  ...
  Survey.findById(surveyId, (err, survey) => {
    ...
    DeleteMany(delete_question_ids)
      .then(() => InsertMany(new_questions))
      .then((questions) => {
      ...
      .then(() => {
        return res.send({
          success: true,
          message: "Survey question successfully updated",
        });
      })
      .catch((err) => {
        return next(err);
